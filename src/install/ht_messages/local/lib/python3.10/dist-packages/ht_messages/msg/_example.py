...
class Metaclass_Example(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('ht_messages')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'ht_messages.msg.Example')
            logger.debug(
...
class Example(metaclass=Metaclass_Example):
    ...
    def __init__(self, **kwargs):
        ...
        self.foo = kwargs.get('foo', int())
        self.bar = array.array('f', kwargs.get('bar', []))
    ...
    def foo(self, value):
        if __debug__:
            assert \
                ...
                "The 'foo' field must be of type 'int'"
            assert value >= 0 and value < 256, \
                "The 'foo' field must be an unsigned integer in [0, 255]"
        self._foo = value
    ...
    def bar(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'f', \
                "The 'bar' array.array() must have the type code of 'f'"
            self._bar = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'bar' field must be a set or sequence and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        self._bar = array.array('f', value)
